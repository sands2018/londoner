...
export function SicBoGame({ desktop, active, onLobby, onSettings }: { desktop: boolean; active: boolean; onLobby: () => void; onSettings: () => void }) {
  ...
  return <main className={`sands-surface sic-game ${desktop ? "is-desktop" : "is-mobile"}`} data-phase={phase} aria-label="骰宝游戏" style={{ width: layout.width, height: layout.height, transform: layout.rotated ? `translate(${size.width - size.right}px,${size.top}px) rotate(90deg) scale(${layout.scale})` : `translate(${size.left}px,${size.top}px) scale(${layout.scale})` }}>
    ...
    <footer className="sic-dock" aria-label="游戏操作">
      ...
      <div className="sic-statistics"><div><button type="button" aria-label="补充虚拟筹码" title="补充虚拟筹码" disabled={animating} onClick={() => setDialog("credits")}>可用筹码 <CirclePlus size={13} /></button><strong>{format(state.balance - wagerTotal(state.bets))}</strong></div><div><span>累计投注</span><strong>{format(state.stats.wagered)}</strong></div><div><span>累计赢利</span><strong className={`sic-profit ${state.stats.profit < 0 ? "negative" : "positive"}`}>{signed(state.stats.profit)}</strong></div></div>
