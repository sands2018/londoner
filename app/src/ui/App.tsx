...
function getSimulatorViewportSize(): { width: number; height: number } {
  return {
...
function clampSimulatorDesktopAnalysisAspect(value: number, viewportWidth: number, viewportHeight: number): number {
  ...
  const maximumAspect = Math.max(
    minimumAspect,
    (Math.max(simulatorDesktopMinimumDragAnalysisWidth, viewportWidth - simulatorDesktopMinimumDragGameWidth - simulatorDesktopSplitterLayoutWidth)) / safeHeight,
  );
...
export function App({ active = true, onReturnToLobby }: { active?: boolean; onReturnToLobby?: () => void } = {}) {
  ...
  const [threeNumberDimCount, setThreeNumberDimCount] = useState<3 | 4 | 5 | 6 | 7 | 8 | null>(() => {
    const saved = localStorage.getItem("londoner.threeNumberDimCount");
    return saved === "3" || saved === "4" || saved === "5" || saved === "6" || saved === "7" || saved === "8"
      ? Number(saved) as 3 | 4 | 5 | 6 | 7 | 8
      : null;
  });
  ...
  const [simulatorDesktopAnalysisAspect, setSimulatorDesktopAnalysisAspect] = useState(() => {
    ...
    const migratedAspect = Number.isFinite(storedGameRatio)
      ? ((1 - storedGameRatio) * viewport.width - simulatorDesktopSplitterLayoutWidth) / viewport.height
      : simulatorDesktopDefaultAnalysisAspect;
  ...
  const shouldComputeChaseSixBenchmarkStats = predictionWindowOpen && predictionTab === "chase6";
  const chaseSixCurrentBettingSpins = Math.max(0, numbers.length - REPEAT_INITIAL_ROUNDS);
  const chaseSixBenchmarkRows = useMemo(
    () => shouldComputeChaseSixBenchmarkStats
  ...
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      const gamePaneWidth = simulatorDesktopAnalysisOpen
        ...
        : viewport.width;
      const availableWidth = Math.max(1, gamePaneWidth - simulatorDesktopViewportPadding);
      const availableHeight = Math.max(1, viewport.height - simulatorDesktopViewportPadding);
  ...
  function closeSimulatorGamePane() {
    if (simulatorUsesDesktopLayout && !simulatorDesktopAnalysisOpen) {
      setSimulatorDesktopAnalysisOpen(true);
    }
    setSimulatorOpen(false);
  ...
  function handleSimulatorDesktopSplitterPointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    simulatorDesktopSplitterDraggingRef.current = true;
  ...
  function stopSimulatorDesktopSplitterDrag(event: ReactPointerEvent<HTMLDivElement>) {
    simulatorDesktopSplitterDraggingRef.current = false;
    ...
    >
      {simulatorDesktopWorkspaceActive && simulatorDesktopAnalysisOpen ? (
          <div
            ...
            aria-valuemax={82}
            aria-valuemin={50}
            aria-valuenow={Math.round(simulatorDesktopGameRatio * 100)}
            className="desktop-workspace-splitter"
            onDoubleClick={() => {
              setSimulatorDesktopAnalysisAspect(simulatorDesktopDefaultAnalysisAspect);
            }}
            onKeyDown={(event) => {
              ...
              const viewport = getSimulatorViewportSize();
              const nextGameWidth = simulatorDesktopGameWidth + (event.key === "ArrowRight" ? 0.02 : -0.02) * viewport.width;
              updateSimulatorDesktopGameRatio(nextGameWidth);
            }}
      ...
      <section className="input-dock" aria-label="号码输入">
        {keyboardMode === "keypad" ? (
          ...
          <div className="digit-entry-grid">
            ...
            <button className="control-button digit-top-import" onClick={openImportDialog} type="button">{homeButtonLabels.input}</button>
            <input
            ...
            <button className="control-button digit-connect" disabled={numbers.length === 0} onClick={openConnectDialog} type="button">{homeButtonLabels.concat}</button>
            <button className="control-button digit-transfer" disabled={sharedLoading || numbers.length === 0} onClick={() => { ensureSharedConnected((u, p) => { setConfirmDialog({ title: "传输数据", message: "要把当前数据上传到传输数据中吗？", confirmFirst: true, confirmText: "上传", onConfirm: () => void uploadCurrentTransfer(u, p) }); }); }} type="button">{homeButtonLabels.pass}</button>
            <button className="control-button digit-save" disabled={numbers.length === 0} onClick={openSaveDialog} type="button">{homeButtonLabels.save}</button>
            <button className="control-button digit-number-zone" onClick={() => { void openDataDialog(); }} type="button">{homeButtonLabels.data}</button>
            ...
            {canUseSimulator ? (
              ...
              <div className="home-game-logo digit-more-game" aria-label="Las Vegas">
        ...
        {keyboardMode !== "digits" ? (
          <div className="dock-actions">
            <button disabled={sharedLoading || numbers.length === 0} onClick={() => { ensureSharedConnected((u, p) => { setConfirmDialog({ title: "传输数据", message: "要把当前数据上传到传输数据中吗？", confirmFirst: true, confirmText: "上传", onConfirm: () => void uploadCurrentTransfer(u, p) }); }); }} type="button">{homeButtonLabels.pass}</button>
            <button disabled={numbers.length === 0} onClick={openConnectDialog} type="button">{homeButtonLabels.concat}</button>
            <button onClick={openImportDialog} type="button">{homeButtonLabels.input}</button>
            ...
            <div className="prediction-body">
              {predictionTab === "quality124" && canUseQuality124 ? (
                <>
                  ...
                  <div className="tabs tabs-top tabs-solid tabs-full quality124-benchmark-tabs" aria-label="124EXT统计范围" role="tablist">
                    <button
                      aria-selected={quality124BenchmarkTab === "current"}
                      className={quality124BenchmarkTab === "current" ? "selected" : ""}
                      onClick={() => { setQuality124BenchmarkTab("current"); localStorage.setItem("londoner.quality124BenchmarkTab", "current"); }}
                      role="tab"
                      type="button"
                    >
                      当前
                    ...
                    <button
                      aria-selected={quality124BenchmarkTab === "history"}
                      className={quality124BenchmarkTab === "history" ? "selected" : ""}
                      onClick={() => { setQuality124BenchmarkTab("history"); localStorage.setItem("londoner.quality124BenchmarkTab", "history"); }}
                      role="tab"
                      type="button"
                    >
                      历史
                  ...
                  {quality124BenchmarkTab === "current" ? (
                    <div className="prediction-roi-table">
                      ...
                      {QUALITY_124_TIER_ORDER.map((tier) => {
                        ...
                        return (
                          <div className="prediction-roi-row quality124-tier-row" key={tier}>
                ...
                <>
                  ...
                  <div className="tabs tabs-top tabs-solid tabs-full chase6-benchmark-tabs" aria-label="追6统计范围" role="tablist">
                    <button
                      aria-selected={chaseSixBenchmarkTab === "current"}
                      className={chaseSixBenchmarkTab === "current" ? "selected" : ""}
                      onClick={() => { setChaseSixBenchmarkTab("current"); localStorage.setItem("londoner.chaseSixBenchmarkTab", "current"); }}
                      role="tab"
                      type="button"
                    >
                      当前
                    ...
                    <button
                      aria-selected={chaseSixBenchmarkTab === "history"}
                      className={chaseSixBenchmarkTab === "history" ? "selected" : ""}
                      onClick={() => { setChaseSixBenchmarkTab("history"); localStorage.setItem("londoner.chaseSixBenchmarkTab", "history"); }}
                      role="tab"
                      type="button"
                    >
                      历史
                ...
                <>
                  ...
                  <div className="tabs tabs-top tabs-solid tabs-full hot-benchmark-tabs" aria-label="热门统计范围" role="tablist">
                    <button
                      aria-selected={hotBenchmarkTab === "current"}
                      className={hotBenchmarkTab === "current" ? "selected" : ""}
                      onClick={() => { setHotBenchmarkTab("current"); localStorage.setItem("londoner.hotBenchmarkTab", "current"); }}
                      role="tab"
                      type="button"
                    >
                      当前
                    ...
                    <button
                      aria-selected={hotBenchmarkTab === "history"}
                      className={hotBenchmarkTab === "history" ? "selected" : ""}
                      onClick={() => { setHotBenchmarkTab("history"); localStorage.setItem("londoner.hotBenchmarkTab", "history"); }}
                      role="tab"
                      type="button"
                    >
                      历史
                  ...
                  {hotBenchmarkTab === "current" ? (
                    <div className="prediction-roi-table">
                      <div className="prediction-roi-row prediction-roi-header hot-calibration-row"><span>档位</span><span>信号</span><span>SP100</span><span>投入</span><span>赢回</span><span>命中</span><span>ROI</span></div>
                      {hotCalibrationBreakdown.map((row) => (
                        <div className="prediction-roi-row hot-calibration-row" key={row.action}>
                          ...
                          <span>{formatSp100(row.stats.signals, hotCurrentBettingSpins)}</span>
...
function formatSp100(signals: number, denominatorSpins: number): string {
  if (denominatorSpins <= 0) return "0.00";
  return ((signals / denominatorSpins) * 100).toFixed(2);
}
